...
// Download NVAPI from https://developer.nvidia.com/nvapi
extern "C" {
...
namespace NvGraphicsCardSwitcher {
	...
	public:
		...
		static int NVidia::IsNVDisplay([Out] System::String^% outDeviceString)
		{
			DWORD iDevNum(0);
			DISPLAY_DEVICE dd;
			dd.cb = sizeof(dd);
			...
			while (EnumDisplayDevices(0, iDevNum, &dd, 0))
			{
				if (dd.StateFlags & DISPLAY_DEVICE_ATTACHED_TO_DESKTOP)
				{
					...
					std::wcout << (iDevNum + 1) << ". --------------------------" << std::endl;

					std::wcout << "DeviceID     = " << ". " << dd.DeviceID << std::endl;
					std::wcout << "DeviceName   = " << ". " << dd.DeviceName << std::endl;
					std::wcout << "DeviceString = " << ". " << dd.DeviceString << std::endl;
					std::wcout << "StateFlags   = " << ". " << dd.StateFlags << std::endl;

					std::wcout << "DeviceID     = " << ". " << dd2.DeviceID << std::endl;
					std::wcout << "DeviceName   = " << ". " << dd2.DeviceName << std::endl;
					std::wcout << "DeviceString = " << ". " << dd2.DeviceString << std::endl;

					std::wcout << "--------------------------" << std::endl;

					std::wstring DeviceString = dd.DeviceString;
					outDeviceString = gcnew String(DeviceString.c_str());
					std::transform(DeviceString.begin(), DeviceString.end(), DeviceString.begin(), ::toupper);
					if (DeviceString.find(L"NVIDIA") != -1) {
						return 0;
					}
				}
				++iDevNum;
		...
		static int NVidia::GetConnectorInfo()
		{
			NvAPI_Status status;
			...
			//NvU32 *displayId;
			//status = NvAPI_DISP_GetDisplayIdByDisplayName("\\DISPLAY1", displayId);
			...
			if ((status == NVAPI_OK) && (pathInfoCount > 0)) {
				pathInfo = new NV_DISPLAYCONFIG_PATH_INFO_V2[pathInfoCount];
		...
		/**
		*  Set current graphics to NV processor
		*  @return 0 - successful
		*          other - error cdoe
		*/
		static int NVidia::SetNVHighPerformance() 
		{
			NvAPI_Status status;
		...
		/**
		*  Set current graphics to Integrated(Intel) processor
		*  @return 0 - successful
		*          other - error cdoe
		*/
		static int NVidia::SetIntegratedGraphics() 
		{
			NvAPI_Status status;
			// (0) Initialize NVAPI. This must be done first of all
			status = NvAPI_Initialize();
		...
		/**
		*  Set current graphics to auto-select processor
		*  @return 0 - successful
		*          other - error cdoe
		*/
		static int NVidia::SetAutoSelectGraphics()
		{
			NvAPI_Status status;
			// (0) Initialize NVAPI. This must be done first of all
			status = NvAPI_Initialize();
